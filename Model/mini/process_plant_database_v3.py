...
df4['植物大类'] = '草本'
df4['类别'] = '草本'
print(f"附件3、4：草种 {len(df3) + len(df4)} 种")
...
for df in [df1, df2, df3, df4]:
    for col in ['拉丁学名', '类别', '生态学特征', '应用场景', '来源', '植物大类']:
        if col not in df.columns:
            df[col] = ''

df1_out = df1[['中文名', '拉丁学名', '植物大类', '类别', '生态学特征', '来源', '应用场景']].copy()
df2_out = df2[['中文名', '拉丁学名', '植物大类', '类别', '生态学特征', '来源', '应用场景']].copy()
df3_out = df3[['中文名', '拉丁学名', '植物大类', '类别', '生态学特征', '来源', '应用场景']].copy()
df4_out = df4[['中文名', '拉丁学名', '植物大类', '类别', '生态学特征', '来源', '应用场景']].copy()
...
flowering_data = {
    '雪松': (1, 12), '广玉兰': (5, 6), '桂花': (9, 10),
    '白玉兰': (3, 4), '二乔玉兰': (3, 4), '深山含笑': (3, 4), '含笑': (3, 4),
    '乐昌含笑': (4, 5), '天竺桂': (9, 10), '樟': (4, 5),
    '杨梅': (4, 5), '枇杷': (11, 12), '柿': (5, 6), '南酸枣': (4, 5),
    '三角枫': (4, 5), '红枫': (4, 5), '鸡爪槭': (4, 5), '元宝枫': (4, 5),
    '枫香': (4, 5), '榔榆': (4, 5), '榉树': (4, 5), '朴树': (4, 5),
    '珊瑚树': (5, 6), '冬青': (5, 6), '铁冬青': (5, 6), '女贞': (5, 6),
    '小叶女贞': (5, 6), '金叶女贞': (4, 5),
    ...
    '冬青卫矛': (6, 7), '金边冬青卫矛': (4, 5),
    '结缕草': (5, 8), '沟叶结缕草': (5, 8),
    '狗牙根': (6, 9), '假俭草': (5, 8), '中华结缕草': (5, 8),
    '麦冬': (7, 8), '阔叶麦冬': (7, 8), '金边麦冬': (7, 8),
    '葱兰': (8, 9), '石蒜': (8, 9), '红花石蒜': (8, 9),
    '玉簪': (6, 7), '鸢尾': (4, 5), '蝴蝶花': (3, 4),
    '蜡梅': (12, 2), '梅': (2, 3), '腊梅': (12, 2),
    '樱花': (3, 4), '垂丝海棠': (3, 4), '西府海棠': (4, 5),
    '贴梗海棠': (3, 4), '北美海棠': (4, 5),
    '桃': (3, 4), '李': (3, 4), '梨': (3, 4), '豆梨': (3, 4),
    '水杉': (3, 4), '池杉': (3, 4), '落羽杉': (3, 4), '墨西哥落羽杉': (3, 4),
    '无患子': (4, 5), '栾树': (6, 7), '黄山栾树': (6, 7),
    '乌桕': (5, 6), '重阳木': (4, 5),
    '臭椿': (5, 6), '香椿': (5, 6), '楝树': (5, 6),
    '合欢': (6, 7), '国槐': (6, 7), '刺槐': (5, 6), '黄檀': (5, 6),
    '枫杨': (4, 5), '七叶树': (4, 5), '椴树': (6, 7),
    '结香': (2, 3),
    '杜英': (6, 7), '红楠': (4, 5), '浙江楠': (4, 5), '阴香': (9, 10),
}
...
def estimate_costs(row):
    ...
    if plant_type == '乔木':
        if any(x in name for x in ['香樟', '悬铃木', '银杏', '朴树', '榉树', '榕树', '枫香']):
            base_cost = np.random.uniform(500, 2000)
        elif any(x in name for x in ['玉兰', '桂花', '樱花', '海棠', '紫薇']):
        ...
        if any(x in name for x in ['杜鹃', '茶梅', '山茶', '绣球']):
        ...
        if any(x in name for x in ['结缕草', '狗牙根', '假俭草', '沟叶结缕草']):
            base_cost = np.random.uniform(3, 15)
            density = 1
        else:
...
def estimate_ecology_score(ecology_text):
    if pd.isna(ecology_text):
        return 6.5
    text = str(ecology_text)
    score = 6.5
...
all_plants['生态适应性评分'] = all_plants['生态学特征'].apply(estimate_ecology_score)
...
def estimate_beauty_score(name, flowering_months):
    ...
    if flowering_months > 0:
        ...
        if flowering_months >= 3:
            score += 0.5
    if any(x in name for x in ['松', '柏', '玉兰', '桂', '梅', '兰', '樱', '海棠', '紫藤', '紫薇']):
        score += 0.5
...
print("【数据预览 - 乔木类（前15种）】")
print("=" * 80)
trees = final_df[final_df['植物大类'] == '乔木'][['中文名', '光照条件', '花期', '单株成本_元', '推荐场景']].head(15)
print(trees.to_string())

print("\n" + "=" * 80)
print("【数据预览 - 草本类（前15种）】")
print("=" * 80)
grasses = final_df[final_df['植物大类'] == '草本'][['中文名', '光照条件', '花期', '每平方米种植成本_元', '推荐场景']].head(15)
print(grasses.to_string())

# 保存为CSV便于查看
final_df.to_csv('data/上海市绿化植物完整数据库.csv', index=True, encoding='utf-8-sig')
print("\nCSV版本已保存: data/上海市绿化植物完整数据库.csv")
